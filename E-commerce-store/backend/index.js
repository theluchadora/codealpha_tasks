...
// DB connection with mongodb (change this to ur relevant db)
...
app.post('/removefromcart', fetchUser, async (req, res) => {
  console.log("Removing itemId:", req.body.itemId);
  const itemId = req.body.itemId.toString();
  
  let userData = await Users.findOne({_id: req.user.id});

  // Check if this item exists in cart and has quantity > 0
  if (userData.cartData[itemId] && userData.cartData[itemId] > 0) {
    ...
    console.log("Cart Data after removal:", userData.cartData);
    res.send("Removed");
  } else {
    res.send("Item not found in cart or already 0");
  }
});
